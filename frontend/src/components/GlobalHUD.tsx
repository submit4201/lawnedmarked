...
const TopSystemBar = ({ agentCount, time }: { agentCount: number, time: Date }) => (
...
const BrandingArea = ({ selectedAgent, onAgentChange }: { selectedAgent: string, onAgentChange: (id: string) => void }) => (
    <div className="flex items-center gap-6">
...
const NavigationTabs = ({ currentView, onNavigate }: { currentView: string, onNavigate: (view: string) => void }) => (
    ...
    <div className="flex items-center gap-6">
        ...
        <div className="border-l border-white/10 pl-6 text-right">
            <div className="flex items-center justify-end gap-2">
                <span className="text-xl font-display text-white">WK {currentWeek}</span>
            ...
            <div className="flex gap-2 justify-end mt-1">
                <motion.button
                    ...
                    className="px-4 py-1 bg-neon-purple/20 border border-neon-purple/50 rounded text-neon-purple text-xs font-bold tracking-widest hover:bg-neon-purple/30 transition-colors uppercase"
                >
                    Next Week
                </motion.button>
                <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    onClick={onRunTurn}
                    className="px-4 py-1 bg-neon-cyan/20 border border-neon-cyan/50 rounded text-neon-cyan text-xs font-bold tracking-widest hover:bg-neon-cyan/30 transition-colors uppercase"
                >
                    Run Turn
                </motion.button>
            </div>
...
export const GlobalHUD: React.FC<GlobalHUDProps> = ({
    ...
    return (
        <div className="flex flex-col w-full z-50 relative shrink-0">
            <TopSystemBar agentCount={Object.keys(game.agents || {}).length} time={currentTime} />
            <header className="px-6 py-4 bg-void/95 backdrop-blur-md border-b border-white/5">
                <div className="flex justify-between items-center">
                    <BrandingArea selectedAgent={selectedAgent} onAgentChange={onAgentChange} />
                    <NavigationTabs currentView={currentView} onNavigate={onNavigate} />
                    <MetricsAndControls
                        cashBalance={game.cash_balance || 0}
                        currentWeek={game.current_week || 1}
