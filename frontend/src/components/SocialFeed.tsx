...
const ReviewCard = ({ event }: { event: any }) => (
...
const DilemmaCard = ({ event }: { event: any }) => (
    <div className="p-3 rounded-lg bg-neon-purple/5 border border-neon-purple/20">
        <div className="flex items-center gap-2 mb-2">
            <AlertCircle size={12} className="text-neon-purple" />
...
const CompetitorCard = ({ event }: { event: any }) => (
...
// Event type to component mapping
const EVENT_CARD_MAP: Record<string, React.FC<{ event: any }>> = {
    'CustomerReviewSubmitted': ReviewCard,
    'DilemmaTriggered': DilemmaCard,
    'CompetitorPriceChanged': CompetitorCard,
};

// Individual feed card component with lookup
const FeedCard = ({ event }: { event: any }) => {
    const CardComponent = EVENT_CARD_MAP[event.event_type];
    return CardComponent ? <CardComponent event={event} /> : null;
};
